...
export default function BulkMatchRow({
  ...
  function renderInput(
    ...
    const error = getFieldError(errors, row.id, field);
    ...
    return (
      <div>
        <input
          ...
          onChange={(event) =>
            onChange(row.id, field, event.target.value)
          }
  ...
  const rowHasError = errors.some((error) => error.rowId === row.id);
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-2">
        {renderInput("round", "text", row.round, "A")}
      </td>
      ...
      <td className="px-3 py-2">
        {renderInput("date", "date", row.date)}
      </td>

      <td className="px-3 py-2">
        {renderInput("time", "time", row.time)}
      </td>
